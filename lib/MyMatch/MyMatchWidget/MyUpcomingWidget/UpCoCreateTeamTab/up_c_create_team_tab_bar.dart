...
import 'choose_captain_and_vicecaptain.dart';
import 'up_c_static_preview.dart';

class UpCoCreateTeamTapBarPage extends StatefulWidget {
  final SaveTeamModel? saveTeamModel;
  final MyMatchModel myMatchModel;

  const UpCoCreateTeamTapBarPage(
      {super.key, this.saveTeamModel,required this.myMatchModel });

  @override
  State<UpCoCreateTeamTapBarPage> createState() => _UpCoCreateTeamTapBarPageState();
}

class _UpCoCreateTeamTapBarPageState extends State<UpCoCreateTeamTapBarPage> {
  @override
  void initState() {
    getTeamsPlayersList();
    super.initState();
            ...
            child: Row(
              ...
              children: [
                  ...
                  onTap: () {
                    if(selectedPlayers.isNotEmpty){
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (context) => UpCoStaticPreview(
                            selectedPlayers: selectedPlayers,
                            myMatchModel: widget.myMatchModel,
                          )));
                    }else{
                      Utils.showToast('please Select player');
                    }
                  },
                  child: Container(
                    width: widths / 2.7,
                ...
                InkWell(
                  onTap: () {
                    if (wkCount <= 0) {
                      Utils.showToast('At-least 1 Wicketkeeper should be selected');
                    } else if (batsmenCount <= 0) {
                      Utils.showToast('At-least 1 Batsman should be selected');
                    } else if (allRounderCount <= 0) {
                      Utils.showToast('At-least 1 AllRounder should be selected');
                    } else if (bowlerCount <= 0) {
                      Utils.showToast('At-least 1 Bowler should be selected');
                    } else if (total != 11) {
                      Utils.showToast('Please Select 11 Players');
                    } else {
                      Navigator.of(context).push(MaterialPageRoute(
                          builder: (context) => ChooseCaptainAndViceCaptain(
                              selectedPlayers: selectedPlayers, myMatchModel: widget.myMatchModel)));
                    }
                  },
                  child: Container(
                    width: widths / 2.7,
            ...
            Container(
              ...
              child: Column(children: <Widget>[
                Container(
                    ...
                    child: Text(
                      'Start time : ${widget.myMatchModel.matchStarted}'
                          .split('.')
                          .first,
                        ...
                        children: <Widget>[
                          CircleAvatar(
                            ...
                            backgroundImage: NetworkImage(
                              '${widget.myMatchModel.teamImage1}',
                            ),
                          ),
                          ...
                          CircleAvatar(
                            ...
                            radius: heights / 40,
                            backgroundImage: NetworkImage(
                              '${widget.myMatchModel.teamImage2}',
                            ),
                          ),
                        ],
                      )),
    ...
    try {
      final response = await http.post(Uri.parse(ApiUrl.playerLists),
          ...
          body: jsonEncode(<String, String>{
            "team1id": '${widget.myMatchModel.teamId1}',
            "team2id": '${widget.myMatchModel.teamId2}'
          }));
